...
namespace utils {
  template <typename T> constexpr void Write(__int64 address, T value) {
    ...
    *reinterpret_cast<T *>(address) = value;
    VirtualProtect(reinterpret_cast<void **>(address), sizeof(T), oldProtection,
                   &oldProtection);
  }

  template <typename K> constexpr K Read(__int64 address) {
    return *reinterpret_cast<K *>(address);
  }

  constexpr uint32_t CalculateChecksum(const uint8_t *data, size_t length) {
    uint32_t checksum = 0;
  ...
  constexpr uint32_t GetTextSectionChecksum(uint64_t hModule) {
    ...
    PIMAGE_NT_HEADERS ntHeaders =
        (PIMAGE_NT_HEADERS)((uint8_t *)hModule + dosHeader->e_lfanew);
    ...
    for (WORD i = 0; i < ntHeaders->FileHeader.NumberOfSections;
      ...
      if (strcmp(sectionName, ".text") == 0) {
        uint8_t *sectionData =
            (uint8_t *)hModule + sectionHeader->VirtualAddress;
