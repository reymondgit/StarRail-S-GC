...
namespace features::speedhack {
	...
	void UpdateWorld(uint64_t game_assembly, uint64_t unity_player) {
		...
		if (globals::world::speed_hack) {
			speed = hooks::game::get_is_in_dialog() || GetAsyncKeyState(VK_CAPITAL) ? globals::world::dialogue_speed : globals::world::global_speed;
		}

		UpdateSpeed(speed, game_assembly, unity_player, false);

		utils::Write<uint8_t>(game_assembly + 0x51292C0, globals::world::peeking ? 0xC3 : 0x40);
		utils::Write<uint8_t>(game_assembly + 0x5800F40, globals::world::invisibility ? 0xC3 : 0x40);
	}
	...
	void UpdateBattle(uint64_t game_assembly, uint64_t unity_player) {
		...
		UpdateSpeed(globals::battle::speed_hack ? globals::battle::battle_speed : 1.f, game_assembly, unity_player, true);

		utils::Write<uint32_t>(game_assembly + 0x5DA5F20, globals::battle::auto_battle_unlock ? 0x90C3C031 : 0x83485340);
	}
}